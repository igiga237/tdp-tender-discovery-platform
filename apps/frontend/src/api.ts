...
/**
 * Updated fetchTendersAPI to include the Authorization header if a token is present.
 */
export const fetchTendersAPI = async (params: Record<string, any>) => {
  try {
    // Retrieve the token from localStorage (same key used in login.tsx)
    ...
    // If there's no data field, treat it as a failure
    if (!response.data) {
      return { success: false, message: "Failed to fetch tenders" };
    }

    // Return the data in the shape expected by TenderSearch
...
// ... the rest of your exports remain unchanged ...
